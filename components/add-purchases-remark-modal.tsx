...
interface AddPurchasesRemarkModalProps {
  open: boolean
...
export function AddPurchasesRemarkModal({
  open,
  onOpenChange,
  purchaseId,
  onRemarkAdded,
}: AddPurchasesRemarkModalProps) {
  ...
  const handleAddRemark = async () => {
    ...
    try {
      ...
      // Get current remarks
      const { data: currentPurchase } = await supabase.from("purchases").select("remarks").eq("id", purchaseId).single()

      let existingRemarks: any[] = []
      if (currentPurchase?.remarks) {
        try {
          existingRemarks = JSON.parse(currentPurchase.remarks)
        } catch {
          existingRemarks = []
        }
      }
      ...
      await supabase.rpc("log_notification", {
        ...
        p_description: `Remark added to purchase record ID: ${purchaseId}`,
        p_ip_address: "",
        p_location: null,
        p_user_agent: typeof window !== "undefined" ? window.navigator.userAgent : "",
        p_meta: { purchaseId, remarkText: remarkInput.trim() },
      })
