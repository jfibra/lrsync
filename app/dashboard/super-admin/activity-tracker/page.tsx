...
export default function ActivityTrackerPage() {
    ...
    function getTopNWithOther<T extends { name: string; value: number; percentage: string }>(
        data: T[],
        n: number
    ): T[] {
        if (data.length <= n) return data;
        const top = data.slice(0, n);
        const otherValue = data.slice(n).reduce((sum, d) => sum + d.value, 0);
        const otherPercentage = (
            data.slice(n).reduce((sum, d) => sum + parseFloat(d.percentage), 0)
        ).toFixed(1);
        return [
            ...top,
            {
        ...
        ? getTopNWithOther(
            ...
            5 // Show top 5 actions
        )
        : [];

    const areaChartData = statistics
        ? getTopNWithOther(
            Object.entries(statistics.areaStats)
                .map(([area, count]) => {
                    let safeArea = area;
                    if (
                        !safeArea ||
                        ...
                        safeArea === null ||
                        safeArea === undefined ||
                        (typeof safeArea === "string" && safeArea.trim() === "")
                    ) {
                        safeArea = "Unassigned";
                    }
                    return {
    ...
    const roleChartData = statistics
        ? getTopNWithOther(
            Object.entries(statistics.roleStats)
                .map(([role, count]) => ({
                    name:
                        !role ||
                            role === "null" ||
