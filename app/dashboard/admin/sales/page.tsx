...
export default function AdminSalesPage() {
  ...
  const fetchSales = async () => {
    try {
      ...
      // Admin-specific filter: only show sales from their assigned area
      if (profile?.role === "admin" && profile.assigned_area) {
        salesQuery = salesQuery.eq("user_assigned_area", profile.assigned_area)
      }
      ...
      // Filter by area if selected (for super_admin, or if admin wants to filter within their area)
  ...
  useEffect(() => {
    fetchSales()
  }, [searchTerm, filterTaxType, filterMonth, filterArea, profile?.assigned_area]) // Added profile.assigned_area to dependencies
