...
export default function SecretaryCommissionReportsPage() {
        ...
        try {
          const presignedRes = await fetch("/api/get-s3-upload-url", {
            ...
            body: JSON.stringify({
              ...
              category: "commission-report-secretary",
  ...
  const exportToExcel = () => {
    ...
    // Create summary data
    const summaryData = [
      [`COMMISSION REPORTS MANAGEMENT REPORT - ${profile?.assigned_area || "Secretary"}`],
      [
        "Generated on:",
    ...
    // Generate filename
    const filename = `Commission_Reports_${profile?.assigned_area?.replace(/\s+/g, "_")}_${new Date().toISOString().split("T")[0]}.xlsx`
    ...
    if (profile?.id) {
      logNotification(supabase, {
        ...
        meta: JSON.stringify({
          ...
          dashboard: "secretary_commission_reports",
